...
Sensor.keywords	= {"lightsensor","lsen","ls","Ls"}
...
function Sensor:execute(input,user)
	...
	if input2 then--input2 should be the id or index of a sensor
		local sensor = lightsensors and (lightsensors[input2] or lightsensors[tonumber(input2)]) or lightsensorIDs and lightsensorIDs[tonumber(input2)]
		...
		return "Please select a light sensor when issuing a sensor command"
	end
end
